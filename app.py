...
def generar_informe_pdf(gdf_analisis, cultivo, analisis_tipo, nutriente, mes_analisis, area_total):
    ...
    try:
        ...
        # Crear un PDF simple de respaldo
        buffer = io.BytesIO()
        c = canvas.Canvas(buffer, pagesize=A4)
        c.drawString(100, 750, f"Informe de Análisis - {cultivo}")
        c.drawString(100, 730, f"Error en generación: {str(e)}")
        c.drawString(100, 710, "Por favor, contacte al soporte técnico")
        c.save()
        buffer.seek(0)
        return buffer
...
def mostrar_resultados():
    ...
    # DESCARGAR RESULTADOS - SECCIÓN CORREGIDA
    ...
    with col2:
        ...
        st.download_button(
            ...
            key="descarga_geojson"
        )
    
    with col3:
        # GENERAR Y DESCARGAR PDF - IMPLEMENTACIÓN CORREGIDA
        ...
        if st.session_state.pdf_generado and st.session_state.pdf_buffer is not None:
            ...
            st.info("📋 El PDF incluye estadísticas, recomendaciones y plan de implementación")
