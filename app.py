...
def calcular_superficie(gdf):
    """Calcula la superficie total en hectáreas"""
    try:
        ...
        return float(area_m2 / 10000)  # Convertir a hectáreas y asegurar que sea float
...
def simular_datos_satelitales(gdf, tipo_pastura, fecha_imagen):
    ...
    for idx, row in gdf_analizado.iterrows():
        ...
        # Calcular área individual de cada sub-lote
        ...
        resultados.append({
            ...
            'geometry': geometry,
            'ndvi': float(ndvi_ajustado),  # Asegurar que sea float
            'tipo_superficie': tipo_superficie,
            'biomasa_disponible_kg_ms_ha': float(biomasa_ajustada),  # Asegurar que sea float
            'area_ha': float(area_ha)  # Asegurar que sea float
        })
...
def calcular_metricas_ganaderas(gdf_analizado, tipo_pastura, peso_promedio, carga_animal):
    ...
    consumo_diario_por_animal = float(peso_promedio * parametros['CONSUMO_PORCENTAJE_PESO'])
    consumo_diario_total = float(consumo_diario_por_animal * carga_animal)
    
    # Inicializar columnas como floats
    ...
    for idx, row in gdf_resultado.iterrows():
        biomasa_disponible_kg = float(row['biomasa_disponible_kg_ms_ha'] * row['area_ha'])
        
        if consumo_diario_por_animal > 0:
            ev_ha = float(biomasa_disponible_kg / consumo_diario_por_animal)
        else:
            ev_ha = 0.0
        
        if consumo_diario_total > 0:
            dias_permanencia = float((biomasa_disponible_kg * parametros['TASA_UTILIZACION_RECOMENDADA']) / consumo_diario_total)
        else:
            dias_permanencia = 0.0
...
def mostrar_sidebar():
    with st.sidebar:
        ...
        # Selector de mapa base con key único
        if FOLIUM_AVAILABLE:
        ...
        # Selección de satélite con key único
        ...
        # Configuración de fechas con key único
        ...
        # Parámetros personalizables con keys únicos
        if tipo_pastura == "PERSONALIZADO":
        ...
        # Parámetros ganaderos con keys únicos
        ...
        # División de potrero con key único
        ...
        # Botón para resetear con key único
...
def mostrar_resultados(gdf_final, area_total, tipo_pastura, base_map_option):
    ...
    with col2:
        ...
        biomasa_promedio = float(gdf_final['biomasa_disponible_kg_ms_ha'].mean())
        ev_total = float(gdf_final['ev_ha'].sum() * area_total / len(gdf_final))
        dias_promedio = float(gdf_final['dias_permanencia'].mean())
