...
def initialize_gee():
    """Inicializa Google Earth Engine usando Service Account o autenticación normal."""
    try:
        # Opción 1: Intentar con Service Account desde secrets de Streamlit
        ...
        if service_account_json:
            ...
            credentials = ee.ServiceAccountCredentials(
                ...
                key_data=service_account_json  # Ya es un string JSON
            )
            ee.Initialize(credentials)
            st.success("✅ Google Earth Engine inicializado con Service Account")
            return True
        else:
            # Opción 2: Inicialización normal (asume que ya se autenticó, por ejemplo, con ee.Authenticate() en local)
            ee.Initialize()
        ...
        st.info("""
        **Para usar Google Earth Engine en producción, necesitas:**
        ...
        2. **Habilitar Earth Engine para la cuenta de servicio:**
           - Ve a [Earth Engine](https://earthengine.google.com/) y registra la cuenta de servicio.
        
        3. **Configurar las credenciales en Streamlit Cloud:**
           - En tu app de Streamlit, ve a "Settings" > "Secrets".
           - Agrega una variable llamada `GEE_SERVICE_ACCOUNT` con el contenido del JSON.
        ...
        **Nota:** En desarrollo local, puedes autenticarte con `ee.Authenticate()` y luego `ee.Initialize()`.
        """)
        return False

# Llamar a la función de inicialización al inicio de la app
