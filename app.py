...
# -----------------------
# FUNCIÓN PDF (PORTADA SOLICITADA)
# -----------------------
def generar_informe_forrajero_pdf(gdf_analizado, tipo_pastura, peso_promedio, carga_animal, mapa_detallado_bytes=None):
    """
    Genera un informe PDF (BytesIO) listo para descargar.
    Título de portada: "INFORME DE DISPONIBILIDAD FORRAJERA PRV"
    """
    buffer = io.BytesIO()
    ...
    try:
        ...
        # Si algo falla con columnas, asignar valores seguros
        area_total = 0
        biomasa_prom = 0
        ndvi_prom = 0
        dias_promedio = 0
...
def calcular_superficie(gdf):
    try:
        # Si CRS es geográfico (grados), aproximación (no perfecta) para convertir a ha
        if gdf.crs and gdf.crs.is_geographic:
            # Evitar usar area en grados directamente; preferible reproyectar en práctica.
            # Aquí usamos aproximación: area (grados^2) * factor para convertir (NO exacto).
...
class DetectorVegetacionRealista:
    ...
    def clasificar_vegetacion_realista(self, ndvi, evi, savi, bsi, ndbi, msavi2=None):
        # Lógica simplificada similar a tu versión original
        if ndvi < 0.12:
    ...
    def calcular_biomasa_realista(self, ndvi, evi, savi, categoria_vegetacion, cobertura, params):
        # Retorna biomasa, crecimiento diario, calidad (simplificado)
...
def analisis_forrajero_completo_realista(gdf, tipo_pastura, peso_promedio, carga_animal, n_divisiones,
    ...
    try:
        ...
        # Sección de exportes: GeoJSON, CSV, PDF
