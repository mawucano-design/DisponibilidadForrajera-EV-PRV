...
with st.sidebar:
    ...
    if tipo_pastura == "PERSONALIZADO":
        ...
        with st.expander("⚙️ Parámetros Avanzados para Detección de Suelo"):
            col1, col2 = st.columns(2)
            with col1:
            ...
            with col2:
                umbral_ndvi_suelo = st.number_input("Umbral NDVI Suelo:", min_value=0.0, max_value=0.5, value=0.15, step=0.01, format="%.2f",
                                                   help="NDVI menor a este valor indica suelo")
                umbral_bsi_suelo = st.number_input("Umbral BSI Suelo:", min_value=0.0, max_value=0.5, value=0.12, step=0.01, format="%.2f",
                                                  help="BSI mayor a este valor indica suelo desnudo")
                penalizacion_suelo = st.number_input("Penalización Suelo:", min_value=0.0, max_value=1.0, value=0.85, step=0.05, format="%.2f",
                                                    help="Factor de reducción de biomasa en áreas de suelo")
            
            sensibilidad_rocoso = st.slider("Sensibilidad Detección Rocoso:", min_value=1, max_value=10, value=7,
                                          help="Mayor valor detecta más áreas rocosas")
    ...
    if tipo_pastura == "PERSONALIZADO":
        ...
        config_data = {
            ...
            'sensibilidad_rocoso': sensibilidad_rocoso
        }
    ...
    if tipo_pastura != "PERSONALIZADO":
        ...
        if config_personalizada:
            ...
            return {
                ...
                'UMBRAL_NDVI_SUELO': config_personalizada.get('umbral_ndvi_suelo', 0.15),
                'UMBRAL_BSI_SUELO': config_personalizada.get('umbral_bsi_suelo', 0.12),
                'UMBRAL_NDBI_SUELO': 0.05,  # Valor por defecto
                'UMBRAL_NDVI_VEGETACION': 0.55,  # Valor por defecto
                'FACTOR_COBERTURA_MAX': 0.90,
                'FACTOR_COBERTURA_MIN': 0.05,
                'PENALIZACION_SUELO': config_personalizada.get('penalizacion_suelo', 0.85),
                'FACTOR_MSAVI2': config_personalizada.get('factor_ndvi', 2000) * 0.9,  # Derivado de NDVI
                'FACTOR_VARI': 700,  # Valor por defecto
                'SENSIBILIDAD_ROCOSO': config_personalizada.get('sensibilidad_rocoso', 7)
            }
        else:
...
# CORREGIDO: ALGORITMO MEJORADO DE CLASIFICACIÓN
def clasificacion_avanzada_suelo_vegetacion(ndvi, bsi, ndbi, evi, savi, msavi2, ui, vari, params):
    """
    Clasificación MEJORADA con mejor detección de suelo rocoso y piedra pelada
    """
    # INICIALIZAR PUNTUACIONES CON PESOS MÁS EQUILIBRADOS
    puntuacion_suelo = 0
    puntuacion_vegetacion = 0
    puntuacion_rocoso = 0
    
    # 1. DETECCIÓN DE SUELO ROCOSO (NUEVO)
    # Suelo rocoso típicamente tiene NDVI bajo pero BSI moderado
    condicion_rocoso = (ndvi < 0.15 and bsi > 0.1 and bsi < 0.3 and vari < 0)
    if condicion_rocoso:
        puntuacion_rocoso += 3
    
    # 2. DETECCIÓN DE PIEDRA PELADA (NUEVO)
    # Áreas muy áridas con múltiples índices bajos
    condicion_piedra = (ndvi < 0.1 and evi < 0.1 and savi < 0.1 and vari < -0.1)
    if condicion_piedra:
        puntuacion_rocoso += 4
    
    # 3. ANÁLISIS DE VEGETACIÓN (MEJORADO)
    # Vegetación densa requiere múltiples confirmaciones
    if ndvi > params['UMBRAL_NDVI_VEGETACION']:
        puntuacion_vegetacion += 2
        # CONFIRMACIÓN con otros índices
        if evi > 0.3:
    ...
    # 4. DETECCIÓN DE SUELO DESNUDO (MEJORADO)
    ...
    # 5. ANÁLISIS DE SUPERFICIES MIXTAS
    if 0.15 <= ndvi <= 0.35:
        # Zona de transición - analizar proporciones
        if bsi > 0.15:
            puntuacion_suelo += 1
        if evi > 0.2:
            puntuacion_vegetacion += 1
    
    # 6. CLASIFICACIÓN FINAL CON LÓGICA MEJORADA
    # PRIORIZAR detección de áreas rocosas
    if puntuacion_rocoso >= 4:
        return "SUELO_ROCOSO_PIEDRA", 0.02, 0.05
    elif puntuacion_rocoso >= 2:
        return "SUELO_ROCOSO_PARCIAL", 0.08, 0.15
    
    # Luego clasificar vegetación vs suelo normal
    diferencia = puntuacion_vegetacion - puntuacion_suelo
    
    if diferencia <= -4:
        return "SUELO_DESNUDO", 0.05, 0.1
    elif diferencia <= -2:
        return "SUELO_PARCIAL", 0.15, 0.25
    elif diferencia <= 0:
        return "VEGETACION_ESCASA", 0.35, 0.5
    elif diferencia <= 2:
        return "VEGETACION_MODERADA", 0.65, 0.75
    else:
        # SOLO clasificar como densa si hay MÚLTIPLES confirmaciones
        if puntuacion_vegetacion >= 4:
            return "VEGETACION_DENSA", 0.85, 1.0
        else:
            return "VEGETACION_MODERADA", 0.70, 0.8

# CORREGIDO: FUNCIÓN DE SIMULACIÓN DE BANDAS MÁS REALISTA
def simular_bandas_sentinel_realista(patron_espacial, tipo_pastura, sensibilidad_rocoso=7):
    """
    Simula bandas Sentinel-2 con MEJOR representación de suelo rocoso
    """
    # INTRODUCIR VARIABILIDAD ESPECÍFICA PARA SUELO ROCOSO
    # Ajustar umbral según sensibilidad
    umbral_rocoso = 0.25 - (sensibilidad_rocoso * 0.02)
    es_rocoso = (patron_espacial < umbral_rocoso)  # Área potencialmente rocosa
    
    if es_rocoso:
        # FIRMA ESPECTRAL DE SUELO ROCOSO/PIEDRA
        blue = 0.18 + np.random.normal(0, 0.02)
        green = 0.20 + np.random.normal(0, 0.03)
        red = 0.25 + np.random.normal(0, 0.04)    # Alta reflectancia en rojo
        nir = 0.30 + np.random.normal(0, 0.05)    # Reflectancia moderada en NIR
        swir1 = 0.35 + np.random.normal(0, 0.06)  # Alta reflectancia en SWIR
        swir2 = 0.32 + np.random.normal(0, 0.05)
...
# CORREGIDO: DETECCIÓN AVANZADA DE SUELO vs VEGETACIÓN
def detectar_suelo_vegetacion_avanzado(blue, green, red, nir, swir1, swir2, params):
    """
    Detección MUY MEJORADA con especial atención a suelo rocoso
    """
    ...
    tipo_superficie, cobertura_base, factor_base = clasificacion_avanzada_suelo_vegetacion(
        ...
        indices['savi'], indices['msavi2'], indices['ui'], indices['vari'], params
    )
    
    # AJUSTES ESPECÍFICOS PARA SUELO ROCOSO
    if "ROCOSO" in tipo_superficie or "PIEDRA" in tipo_superficie:
        # Suelo rocoso tiene BIOMASA MUY BAJA
        cobertura_final = max(0.01, min(0.1, cobertura_base))
        factor_penalizacion = 0.02 if "PIEDRA" in tipo_superficie else 0.08
        
        # Validación adicional para rocoso
        if indices['ndvi'] > 0.2 and tipo_superficie.startswith("SUELO_ROCOSO"):
            # Posible error - revisar clasificación
            if indices['evi'] > 0.25:
        ...
        # Ajustes basados en índices (más conservadores)
        if indices['ndvi'] > 0:
            cobertura_ajustada += indices['ndvi'] * 0.15  # Reducido de 0.2
        
        if indices['evi'] > 0.2:
            cobertura_ajustada += indices['evi'] * 0.1   # Reducido de 0.15
        
        # Penalizaciones más agresivas para suelo
        if indices['bsi'] > 0.1:
            cobertura_ajustada -= indices['bsi'] * 0.4   # Aumentado de 0.3
        
        # LIMITES MÁS ESTRICTOS por tipo
        if tipo_superficie == "SUELO_DESNUDO":
            factor_penalizacion = 0.05
            cobertura_final = max(0.02, min(0.1, cobertura_ajustada))
        elif tipo_superficie == "SUELO_PARCIAL":
            factor_penalizacion = 0.20  # Reducido de 0.25
            cobertura_final = max(0.1, min(0.25, cobertura_ajustada))
        elif tipo_superficie == "VEGETACION_ESCASA":
            factor_penalizacion = 0.40  # Reducido de 0.45
            cobertura_final = max(0.25, min(0.5, cobertura_ajustada))
        elif tipo_superficie == "VEGETACION_MODERADA":
            factor_penalizacion = 0.70  # Reducido de 0.75
            cobertura_final = max(0.5, min(0.75, cobertura_ajustada))
        else:  # VEGETACION_DENSA
            factor_penalizacion = 0.90  # Reducido de 1.0
            cobertura_final = max(0.75, min(0.95, cobertura_ajustada))
    
    # VALIDACIÓN CRUZADA FINAL MÁS ESTRICTA
    if tipo_superficie.startswith("VEGETACION"):
        # Vegetación debe tener al menos un índice de vegetación decente
        if indices['ndvi'] < 0.25 and indices['evi'] < 0.15:
            # Probablemente no es vegetación real
...
def calcular_indices_forrajeros_gee(gdf, tipo_pastura, params):
    ...
    for idx, row in gdf_centroids.iterrows():
        ...
        sensibilidad = params.get('SENSIBILIDAD_ROCOSO', 7)
        blue, green, red, nir, swir1, swir2 = simular_bandas_sentinel_realista(patron_espacial, tipo_pastura, sensibilidad)
        ...
        if "ROCOSO" in tipo_superficie or "PIEDRA" in tipo_superficie:
            # Biomasa MUY reducida en áreas rocosas
            biomasa_base = params['MS_POR_HA_OPTIMO'] * 0.05 * cobertura_vegetal
            crecimiento_diario = params['CRECIMIENTO_DIARIO'] * 0.05
            calidad_forrajera = 0.05
            
        elif tipo_superficie in ["SUELO_DESNUDO", "SUELO_PARCIAL"]:
            # Biomasa muy reducida en áreas de suelo
        ...
        if "ROCOSO" in tipo_superficie or "PIEDRA" in tipo_superficie:
            prob_suelo_desnudo = 0.98
        elif tipo_superficie == "SUELO_DESNUDO":
...
# CORREGIDO: FUNCIÓN MEJORADA PARA MAPA DE COBERTURA
def crear_mapa_cobertura(gdf, tipo_pastura):
    """Crea mapa MEJORADO de cobertura vegetal y tipos de superficie"""
    try:
        fig, ax = plt.subplots(1, 1, figsize=(14, 10))
        
        # COLORES MEJORADOS PARA DIFERENCIAR SUELO ROCOSO vs VEGETACIÓN
        colores_superficie = {
            'SUELO_ROCOSO_PIEDRA': '#5a5a5a',      # Gris oscuro - piedra pelada
            'SUELO_ROCOSO_PARCIAL': '#8c8c8c',     # Gris medio - suelo rocoso
        ...
        ax.set_title(f'🌱 MAPA DE COBERTURA VEGETAL - {tipo_pastura}\n'
                    f'Detección Avanzada de Suelo Rocoso vs Biomasa Forrajera', 
...
def crear_dashboard_metricas(gdf_analizado):
    ...
    try:
        ...
        if 'tipo_superficie' in gdf_analizado.columns:
            ...
            colors = ['#5a5a5a', '#8c8c8c', '#8c510a', '#bf812d', '#dfc27d', '#80cdc1', '#01665e']
...
def crear_resumen_ejecutivo(gdf_analizado, tipo_pastura, area_total, params):
    ...
    area_rocoso = area_por_tipo.get('SUELO_ROCOSO_PIEDRA', 0) + area_por_tipo.get('SUELO_ROCOSO_PARCIAL', 0)
...
• Área Rocosa/Piedra: {area_rocoso:.1f} ha ({(area_rocoso/area_total*100):.1f}%)
...
"""
    ...
    for tipo in ['SUELO_ROCOSO_PIEDRA', 'SUELO_ROCOSO_PARCIAL', 'SUELO_DESNUDO', 'SUELO_PARCIAL', 'VEGETACION_ESCASA', 'VEGETACION_MODERADA', 'VEGETACION_DENSA']:
...
"""
    ...
    if area_rocoso > area_total * 0.2:
        resumen += "• ALTA PROPORCIÓN ROCOSA: Considerar exclusión de áreas rocosas del pastoreo\n"
